...
final class NamePrettifier
{
    ...
    public function prettifyTestClass(string $className): string
    {
        ...
        if (substr($className, -1 * strlen('CeshiTest')) === 'CeshiTest') {
            $className = substr($className, 0, strlen($className) - strlen('CeshiTest'));
        }

        if (strpos($className, 'Tests') === 0) {
            $className = substr($className, strlen('Tests'));
        } elseif (strpos($className, 'CeshiTest') === 0) {
            $className = substr($className, strlen('CeshiTest'));
        }
