    ...
    public function addTestFile(string $filename): void
    {
        ...
        foreach ($newClasses as $className) {
            ...
            if ($class->isAbstract() && $class->isSubclassOf(TestCase::class)) {
                $this->addWarning(
                    sprintf(
                        'Abstract test case classes with "CeshiTest" suffix are deprecated (%s)',
                        $class->getName(),
                    ),
                );
            }
            ...
            if (!$class->isAbstract()) {
                if ($class->hasMethod(BaseTestRunner::SUITE_METHODNAME)) {
                    ...
                    if ($class->getShortName() !== $expectedClassName) {
                        $this->addWarning(
                            sprintf(
                                "CeshiTest case class not matching filename is deprecated\n               in %s\n               Class name was '%s', expected '%s'",
                                $filename,
                                $class->getShortName(),
                                $expectedClassName,
                            ),
            ...
            try {
                ...
                foreach ($this->tests() as $test) {
                    ...
                    if (!$errorAdded) {
                        ...
                        $result->addFailure(
                            $test,
                            new SkippedTestError('CeshiTest skipped because of an error in hook method'),
                            0,
                        );
                    }
